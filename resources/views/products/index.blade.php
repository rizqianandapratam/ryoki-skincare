    ...
    <div class="bg-white p-4 md:p-6 rounded-2xl border border-slate-200 shadow-xs space-y-4 md:space-y-0 md:flex md:flex-row md:justify-between md:items-center md:gap-4">
        ...
        <div class="flex items-center gap-2 overflow-x-auto w-full md:w-auto pb-2 md:pb-0 scrollbar-hide">
            ...
            <template x-for="cat in categories" :key="cat">
                <button @click="setCategory(cat)"
                        :class="activeCategory.toLowerCase() === cat.toLowerCase()
                            ? 'bg-[#0284C7] text-white shadow-sm shadow-sky-500/25 scale-[1.03]'
                            : 'bg-slate-100 text-slate-600 hover:bg-slate-200 hover:text-slate-800'"
        ...
        <div class="relative w-full md:w-72">
            <input type="text"
                   ...
                   class="w-full pl-10 pr-10 py-2.5 text-xs rounded-xl border border-slate-200 focus:border-[#0284C7] focus:ring-2 focus:ring-sky-100 transition-all duration-200 bg-white"
...
<script>
    function productCatalog() {
        return {
            allProducts: @json($initialProducts ?? []),
            ...
            setCategory(cat) {
                if (this.activeCategory.toLowerCase() === cat.toLowerCase()) {
                    this.activeCategory = '';
                } else {
            ...
            filterProducts() {
                const cat = this.activeCategory.trim().toLowerCase();
                const q = this.searchQuery.trim().toLowerCase();

                this.products = this.allProducts.filter(p => {
                    const matchCategory = !cat || (p.category && p.category.toLowerCase() === cat);
                    const matchSearch = !q ||
